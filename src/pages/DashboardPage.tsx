...
export function DashboardPage() {
  ...
  return (
    <PageWrapper>
      <div className="relative overflow-hidden rounded-xl sm:rounded-2xl shadow-md bg-gradient-to-l from-[#093F85] via-[#020F1F] to-[#041227] text-white p-4 sm:p-7 flex flex-col lg:flex-row-reverse gap-4 sm:gap-6 lg:gap-10 items-stretch mb-4 sm:mb-6">
        <div className="w-full lg:w-72 xl:w-80 flex flex-col gap-3 order-2 lg:order-1">
          ...
          <div className="rounded-xl bg-white/5 border border-white/10 p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center flex-shrink-0 text-amber-300 text-lg">★</div>
          ...
          <div className="rounded-xl bg-white/5 border border-white/10 p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center flex-shrink-0 text-base">💼</div>
            <div className="flex-1 min-w-0">
        ...
        <div className="flex-1 flex flex-col justify-center text-center lg:text-right order-1 lg:order-2">
          ...
          <h1 className="text-xl sm:text-3xl font-bold mb-2">أهلا بك في لوحة الاداء الخاصة بك</h1>
          ...
          <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3">
            <div className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white/10 backdrop-blur text-xs sm:text-sm">
              <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[var(--bidex-primary)]/90 text-[10px] text-white">🏆</span>
              <span>
            ...
            <Link
              to="/sales/new"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-[#093F85] text-sm font-semibold shadow-sm hover:brightness-110 transition-all"
            >
      ...
      {isAdmin && dashboard?.expired_inventory && dashboard.expired_inventory.products_count > 0 && (
        <div
          className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100"
          role="status"
        >
      ...
      {dashboard?.near_expiry_products && dashboard.near_expiry_products.count > 0 && (
        <div
          className="mb-4 rounded-xl border border-sky-300 bg-sky-50 px-4 py-3 text-sm text-sky-950 dark:border-sky-800 dark:bg-sky-950/35 dark:text-sky-100"
          role="alert"
        >
          ...
          <Link
            ...
            className="mt-2 inline-flex rounded-lg bg-[var(--bidex-primary)] px-3 py-1.5 text-xs font-semibold text-white hover:brightness-110"
          >
        ...
        <div className={pageCardShell}>
          <div className={`${pageCardInner} py-3 px-4`}>
            ...
            {(dashboard?.near_expiry_products?.count ?? 0) > 0 && (
              <Link
                to="/products/near-expiry"
                className="mt-2 inline-block text-xs font-semibold text-[var(--bidex-primary)] hover:underline dark:text-sky-400"
              >
                فتح القائمة ←
              </Link>
            )}
      ...
      {loyaltyHighlight && (
        <div className={`${pageCardShell} mb-3`}>
          <div className={`${pageCardInner} py-3 px-4`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[65%_35%] gap-5">
        <div className="flex flex-col gap-5">
          <div className={`${pageCardInner} p-5 flex flex-col gap-4 rounded-xl shadow-md border border-card h-[328px] sm:h-[360px]`}>
            <div className="flex items-center justify-between mb-2 flex-shrink-0">
              <div className="relative" ref={revenueDropdownRef}>
                <button
                  type="button"
                  onClick={() => setRevenueDropdownOpen((o) => !o)}
                  className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-muted-bg hover:opacity-90 text-xs font-semibold transition-colors"
        ...
        <div className={`${pageCardInner} p-5 flex flex-col gap-4 rounded-xl shadow-md border border-card h-[328px] sm:h-[360px]`}>
          ...
          <div className="flex items-center gap-6 flex-1 min-h-0">
            <div className="shrink-0 w-[140px] h-[140px] min-w-[140px] min-h-[140px]">
            ...
            <div className="flex-1 space-y-2 text-xs sm:text-sm">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-[var(--bidex-primary)]" />
      ...
      {lineData.length > 0 && (
        <div className={`${pageCardShell} mt-3`}>
